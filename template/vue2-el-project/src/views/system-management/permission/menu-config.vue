...
export default {
    ...
    methods: {
        ...
        edit_menu(node, data){ //  编辑菜单
            this.tempMenu[node.id] = deep_copy(data);
            this.$set(data, 'edit', 1);
        },
        cancel_menu(node, data){ //  取消编辑菜单
            data.edit = 0;
            data.auth_name = this.tempMenu[node.id].auth_name;
            data.path = this.tempMenu[node.id].path;
        },
        ...
        delete_submit(){ //  确认删除菜单
            if (this.deleteConfirmPop.data.id.includes('default-')){
                ...
                if (this.deleteConfirmPop.data.path_type == '2'){
                    this.$elMessage.success('接口删除成功');
                } else {
                    this.$elMessage.success('菜单删除成功');
                }
        ...
        save_menu(node, data){ //  保存菜单
            if (data.auth_name.trim() == ''){
                this.$elMessage.error('标题不能为空');
                return false;
            }
            if (data.path.trim() == ''){
                this.$elMessage.error('url地址不能为空');
                return false;
            }
            ...
            if (data.id.includes('default-')){
                ...
                api_manage_auth_create(_data).then(res => {
                    if (res.code == 200){
                        ...
                        if (_data.path_type == '2'){
                            this.$elMessage.success('接口新增成功');
                        } else {
                            this.$elMessage.success('菜单新增成功');
                        }
                    } else {
                ...
                api_manage_auth_update(_data).then(res => {
                    if (res.code == 200){
                        if (_data.path_type == '2'){
                            this.$elMessage.success('接口修改成功');
                        } else {
                            this.$elMessage.success('菜单修改成功');
                        }
                    } else {
                        this.$refs['ref-tree-data'].remove(data);
                    }
