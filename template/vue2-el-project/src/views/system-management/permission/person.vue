...
import qmFormPop from '@/component/qm-form-pop';
...
export default {
    ...
    methods: {
        ...
        delete_user_submit(){ // 删除用户
            api_manage_user_delete({id: this.confirmPop.data.id}).then(res => {
                if (res.code == 200){
                    this.tableData.splice(this.confirmPop.index, 1);
                    this.$elMessage.success('人员删除成功');
                }
        ...
        add_user_submit(){ // 添加用户
            ...
            api_manage_user_create({
                ...
                if (res.code == 200){
                    this.$elMessage.success('新用户添加成功');
                    this.init_data();
                    this.addEditUserDataPop.visible = false;
                }
            });
        },
        update_user_submit(){ // 编辑用户
            if (!this.filter_form_data()){
                return false;
            }
            api_manage_user_update({
                ...
                if (res.code == 200){
                    this.$elMessage.success('用户信息编辑成功');
                    this.init_data();
                    this.addEditUserDataPop.visible = false;
                }
            });
        ...
        filter_form_data(){ // 检测表单数据
            ...
            if (this.formData.username == ''){
                this.$elMessage.error('人员姓名不能为空');
                return false;
            }
            if (this.formData.email == ''){
                this.$elMessage.error('邮箱不能为空');
                return false;
            }
            if (!this.formData.email.includes('@')){
                this.$elMessage.error('请填写正确的邮箱格式');
                return false;
            }
            return true;
        }
